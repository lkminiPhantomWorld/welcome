...
def check_manifest_and_routes() -> None:
    ...
    if manifest.get("root_protocol") != ROOT_PROTOCOL: fail("Manifest 現行根協議不正確")
    ...
    if entry.get("root_protocol") != ROOT_PROTOCOL: fail("Manifest 入口根協議不正確")
...
def check_entry_semantics() -> None:
    ...
    required_reader=("🪞幻影世界 Reader","🥃老K系統入口","LKMINI","唯一入口 Reader / Gateway",ROOT_PROTOCOL,ROOT_SHA256,"A=A","拒絕掛載 · 不得進入","const allPass=shaMatch&&rootProtocolPass&&locatorPass&&payloadPass&&identityPass&&axiomPass")
...
def check_locator_and_reverse_chain() -> None:
    ...
    if locator.get("root_protocol") != ROOT_PROTOCOL: fail("Locator 現行根協議不正確")
    ...
    if entry.get("root_protocol") != ROOT_PROTOCOL: fail("Locator Reader 根協議不正確")
    ...
    if reverse.get("root_protocol") != ROOT_PROTOCOL: fail("ReverseChain 現行根協議不正確")
    ...
    if nodes[0].get("root_protocol") != ROOT_PROTOCOL: fail("ReverseChain Reader 根協議不正確")
...
def check_current_protocol() -> None:
    current_files = (WELCOME, READER, MANIFEST, LOCATOR, REVERSE)
    old_protocols = ("幻界" + "://", "mini:" + chr(92))
    for path in current_files:
        text = path.read_text(encoding="utf-8")
        if ROOT_PROTOCOL not in text: fail(f"現行檔案缺少根協議：{path.relative_to(ROOT)}")
        for old in old_protocols:
            if old in text: fail(f"現行檔案仍引用舊根協議：{path.relative_to(ROOT)}")
...
def main() -> None:
    ...
    check_hashes(); check_manifest_and_routes(); check_entry_semantics(); check_locator_and_reverse_chain(); check_current_protocol()
    ...
    print("PASS: ROOT_PROTOCOL=LKMINI://")
    print("PASS: A=A")
