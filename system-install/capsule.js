...
let db, sessionKey=null, currentOwner='', recorder=null, chunks=[];
...
function openDB(){
 return new Promise((resolve,reject)=>{
  ...
  r.onupgradeneeded=()=>{
   const d=r.result;
   ['chat','voice','events','files','secrets','meta'].forEach(n=>{
...
async function unlockCapsule(){
 ...
 if(meta && (meta.owner!==owner || meta.passHash!==hash)){
   qs('lockStatus').innerHTML='<div class="status dangerbox">名字或密語不正確。</div>';return;
 }
...
function openModule(id){document.querySelectorAll('section.module').forEach(x=>x.classList.remove('active'));qs(id).classList.add('active');window.scrollTo({top:0,behavior:'smooth'})}
...
async function startRecording(){
 try{
  ...
  recorder.onstop=async()=>{
   ...
   stream.getTracks().forEach(t=>t.stop());qs('voiceTitle').value='';qs('recordBtn').classList.remove('hidden');qs('stopBtn').classList.add('hidden');qs('voiceStatus').textContent='已保存。';
   await renderVoices();await renderSummary();
  };
  recorder.start();qs('recordBtn').classList.add('hidden');qs('stopBtn').classList.remove('hidden');qs('voiceStatus').textContent='錄音中……';
 }catch(e){qs('voiceStatus').textContent='無法啟用麥克風：'+e.message}
}
function stopRecording(){if(recorder&&recorder.state!=='inactive')recorder.stop()}
async function renderVoices(){
 if(!db)return;const arr=(await getAll('voice')).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
 qs('voiceList').innerHTML=arr.length?arr.map(x=>{const url=URL.createObjectURL(new Blob([unb64(x.data)],{type:x.type}));return `<div class="item"><div class="meta">${new Date(x.createdAt).toLocaleString()}</div><strong>${esc(x.title)}</strong><audio controls src="${url}"></audio><button class="danger" onclick="removeItem('voice','${x.id}')">刪除</button></div>`}).join(''):'<p class="small">還沒有語音。</p>';
}
...
async function downloadStoredFile(id){const x=(await getAll('files')).find(v=>v.id===id);downloadBlob(new Blob([unb64(x.data)],{type:x.type}),x.name)}
...
async function saveMeetingLink(){const all=await getAll('meta'),m=all.find(x=>x.id==='identity');m.meetingLink=qs('meetingLink').value.trim();await put('meta',m);alert('已保存')}
function openMeetingLink(){const u=qs('meetingLink').value.trim();if(!u)return;location.href=u}

async function collectAll(){
 const stores=['chat','voice','events','files','secrets','meta'],data={format:'PhantomCapsule',exportedAt:nowISO(),stores:{}};
 for(const s of stores)data.stores[s]=await getAll(s);return data;
}
...
async function exportCapsule(){
 const data=await collectAll();const blob=new Blob([JSON.stringify(data)],{type:'application/json'});cachedExportFile=new File([blob],'🪞個人幻影膠囊備份.json',{type:'application/json'});
...
async function shareCapsule(){
 const data=await collectAll();const file=new File([JSON.stringify(data)],'🪞個人幻影膠囊備份.json',{type:'application/json'});
 try{if(navigator.canShare&&navigator.canShare({files:[file]}))await navigator.share({title:'個人幻影膠囊',files:[file]});else{downloadBlob(file,file.name);alert('此瀏覽器未提供檔案分享，已改為下載。')}}catch(e){}
}
...
async function importCapsuleFile(file,atLock){
 if(!file)return;
 try{
  const data=JSON.parse(await file.text());if(data.format!=='PhantomCapsule')throw new Error('格式不符');
  await openDB();for(const [s,arr] of Object.entries(data.stores||{})){if(!db.objectStoreNames.contains(s))continue;for(const x of arr)await put(s,x)}
...
async function eraseAll(){
 if(!confirm('這會清除本機全部資料。請先匯出備份。確定繼續？'))return;
 for(const s of ['chat','voice','events','files','secrets','meta'])await clearStore(s);sessionStorage.clear();location.reload();
}
